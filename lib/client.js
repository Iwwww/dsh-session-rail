 ...
 * button — DSH owns the row's position through its panel ordering contract.
 * The row exists only while the rail is collapsed, so a wide sidebar
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			"[data-sidebar-collapsed] [data-slot='sidebar.workspaces'] [aria-label='Add workspace']{display:none}",
		...
		function RailGlyph(props) {
			...
			/**
			 * Own the panellist row's interactions. DSH owns its placement and order;
			 * this effect only adapts the registered panel button into a flyout trigger.
			 */
			React.useEffect(() => {
		...
		function apply(ctx) {
			...
			let disposeRow = null;
			const isSidebarCollapsed = () => document.querySelector('[data-slot="root"] [data-sidebar-collapsed]') !== null;
			const syncRow = () => {
			...
			let frame = document.querySelector('[data-slot="root"]');
			const mount = document.querySelector("#root");
			if (mount === null) return;
			const frameObserver = new MutationObserver(() => {
				if (frame === null) {
					frame = document.querySelector('[data-slot="root"]');
					if (frame === null) return;
					frameObserver.disconnect();
					frameObserver.observe(frame, { attributes: true, subtree: true, attributeFilter: ["data-sidebar-collapsed"] });
					syncRow();
					return;
				}
				if (isSidebarCollapsed() !== (disposeRow !== null)) syncRow();
			});
			if (frame === null) {
				// Attach to the application mount only until DSH's root frame appears.
				frameObserver.observe(mount, { childList: true, subtree: true });
			} else {
				// DSH publishes collapse state on AppFrame; keep steady-state observation there.
				frameObserver.observe(frame, { attributes: true, subtree: true, attributeFilter: ["data-sidebar-collapsed"] });
			}
			ctx.effect(() => () => frameObserver.disconnect(), "session-rail: fold watcher");
			syncRow();
		}
