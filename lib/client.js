 ...
 * The flyout itself:
 *
 * - the current Session's project is open by default, and only one project is
 *   ever open (hover swaps it, a click on the open one collapses it);
 * - the panel's height is frozen when it opens, so switching projects scrolls
 *   inside a stable box instead of moving the edge away from the pointer;
 ...
 * - a project row offers "new session in this project" on hover, and a chat row
 *   offers fork, rename and archive;
 * - the list is a `tree`: arrows move, Enter activates, and the search box hands
 *   focus to the list on ArrowDown.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Cap on projects rendered before the "more projects" row. */
		const MAX_PROJECTS = 6;
		/** Cap on chats rendered per project until its "more" row raises the list. */
		const MAX_CHATS = 5;
		/** Cap on chats once the list is raised, or while a query is active. */
		const MAX_CHATS_EXPANDED = 25;
		...
		const zh = {
			...
			"empty.noMatch": "没有匹配项",
			loading: "加载中",
			"more.chats": "还有 {count} 条",
			"more.projects": "还有 {count} 个项目",
			ungrouped: "未分组",
			close: "关闭",
		...
		const en = {
			...
			loading: "Loading",
			"more.chats": "{count} more",
			"more.projects": "{count} more projects",
			ungrouped: "No project",
			close: "Close",
			"project.new": "New session in this project",
			"actions.chat": "Session actions",
			"action.fork": "Fork",
		...
		const ru = {
			...
			loading: "Загрузка",
			"more.chats": "Ещё {count}",
			"more.projects": "Ещё {count} проектов",
			ungrouped: "Без проекта",
			close: "Закрыть",
			"project.new": "Новый чат в этом проекте",
			"actions.chat": "Действия с чатом",
			"action.fork": "Форк",
		...
		const CSS = [
			...
			".dsh-rail-sessions__panel{position:fixed;z-index:80;display:flex;flex-direction:column;width:320px;overflow:hidden;border:1px solid var(--dsw-alias-border-l2,rgba(127,127,127,.28));border-radius:12px;background:var(--dsw-alias-bg-overlay,var(--dsw-alias-bg-layer-2,#1c1c1e));box-shadow:0 14px 36px rgba(0,0,0,.28);color:var(--dsw-alias-label-primary,inherit);font-size:13px;line-height:1.35;transform-origin:top left;animation:dsh-rail-flyout-in 150ms cubic-bezier(.2,.8,.2,1)}",
			...
			"@media (prefers-reduced-motion:reduce){.dsh-rail-sessions__panel,.dsh-rail-sessions__panel.is-closing,.dsh-rail-sessions__chats,.dsh-rail-sessions__skeleton{animation:none}.dsh-rail-sessions__chevron{transition:none}}",
			...
			".dsh-rail-sessions__list{flex:1;min-height:180px;overflow-y:auto;padding:4px 6px 8px;outline:none;scrollbar-color:var(--dsh-scrollbar-thumb,rgba(127,127,127,.4)) transparent}",
			...
			".dsh-rail-sessions__project{display:flex;align-items:center;gap:6px;width:100%;padding:6px 8px;border:0;border-radius:8px;background:transparent;color:var(--dsw-alias-label-secondary,inherit);font:inherit;text-align:left;cursor:pointer}",
			".dsh-rail-sessions__project:hover,.dsh-rail-sessions__project.is-open{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.14));color:var(--dsw-alias-label-primary,inherit)}",
			".dsh-rail-sessions__project.is-active{box-shadow:inset 0 0 0 2px var(--dsw-alias-brand-primary,#4d6bfe)}",
			".dsh-rail-sessions__chevron{flex:none;opacity:.55;transition:transform .16s cubic-bezier(.2,.8,.2,1)}",
			".dsh-rail-sessions__project.is-open .dsh-rail-sessions__chevron{transform:rotate(90deg)}",
			...
			".dsh-rail-sessions__chat{display:flex;align-items:center;gap:6px;width:100%;padding:5px 8px 5px 22px;border:0;border-radius:8px;background:transparent;color:var(--dsw-alias-label-secondary,inherit);font:inherit;text-align:left;cursor:pointer}",
			...
			".dsh-rail-sessions__iconButton{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;padding:0;border:0;border-radius:6px;background:transparent;color:var(--dsw-alias-label-tertiary,inherit);cursor:pointer}",
			...
			".dsh-rail-sessions__more{display:block;width:100%;padding:5px 8px 5px 22px;border:0;border-radius:8px;background:transparent;color:var(--dsw-alias-brand-primary,inherit);font:inherit;text-align:left;cursor:pointer}",
			".dsh-rail-sessions__more:hover,.dsh-rail-sessions__more.is-active{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.14))}",
		 ...
		 * @returns projects in registry order, then one "no project" group.
		 */
		...
		function RailBrowser(props) {
			...
			/** The single open project: hovering another one swaps it in place. */
			const [activeId, setActiveId] = React.useState(null);
			/** Whether the operator closed the active project by clicking its row. */
			const [collapsed, setCollapsed] = React.useState(false);
			/** Whether the active project shows its full recent list. */
			...
			const currentGroupId = React.useMemo(() => {
				const current = sessions === undefined || sessions === null ? undefined : sessions.current;
				if (current === undefined || current === null) return null;
				for (const group of groups) {
					if (group.items.some((row) => row.id === current)) return group.id;
				}
				return null;
			}, [groups, sessions]);
			React.useEffect(() => {
				setActiveId((previous) => {
					if (previous !== null && groups.some((group) => group.id === previous)) return previous;
			...
			/** Swap the open project, and drop the raised list with it. */
			const openProject = (groupId) => {
				if (groupId === activeId && !collapsed) return;
				setActiveId(groupId);
				setCollapsed(false);
				setRaised(false);
			};
			...
			if (searching) {
				...
				const limitProjects = asPanel || showAll ? groups.length : MAX_PROJECTS;
				const visibleGroups = groups.slice(0, limitProjects);
				const hiddenProjects = groups.length - visibleGroups.length;
				body = [];
				for (const group of visibleGroups) {
					const isPinned = asPanel;
					const isOpen = isPinned || (group.id === activeId && !collapsed);
					const projectKey = "p:" + group.id;
					nodes.push({ key: projectKey, kind: "project", group, isOpen });
					const limit = isPinned || raised ? MAX_CHATS_EXPANDED : MAX_CHATS;
					const shown = isOpen ? group.items.slice(0, limit) : [];
					...
					const projectChildren = [
						React.createElement("div", {
							...
							id: rowDomId(projectKey),
							className: "dsh-rail-sessions__project" + (isOpen ? " is-open" : "") + (activeKey === projectKey ? " is-active" : ""),
							tabIndex: -1,
							onClick: () => {
								setActiveKey(projectKey);
								if (group.id === activeId && !collapsed) {
									setCollapsed(true);
									return;
								}
							...
							icon(["IconChevronRightOutline14", "IconChevronRightOutline"], { size: 12, className: "dsh-rail-sessions__chevron" }),
							icon(["IconFolderOpen16", "IconFolderOpen"], { size: 15 }),
							React.createElement("span", { className: "dsh-rail-sessions__projectTitle" }, group.title === null ? t("ungrouped") : group.title),
					...
					body.push(React.createElement("div", { key: group.id, className: "dsh-rail-sessions__group" }, projectChildren));
					if (shown.length > 0) {
						...
						projectChildren.push(React.createElement("div", { key: "chats", role: "group", className: "dsh-rail-sessions__chats" }, chatNodes));
					}
					if (isOpen && rest > 0) {
						const key = "m:" + group.id;
						nodes.push({ key, kind: "moreChats", group });
						projectChildren.push(React.createElement("div", {
							key: "more",
							role: "treeitem",
							"aria-level": 2,
							id: rowDomId(key),
							tabIndex: -1,
							className: "dsh-rail-sessions__more" + (activeKey === key ? " is-active" : ""),
							onClick: () => setRaised(true),
							onMouseEnter: () => setActiveKey(key)
						}, t("more.chats", { count: rest })));
					}
				}
				if (hiddenProjects > 0) {
					const key = "mp";
					nodes.push({ key, kind: "moreProjects" });
					body.push(React.createElement("div", {
						...
						onMouseEnter: () => setActiveKey(key)
					}, t("more.projects", { count: hiddenProjects })));
				}
			}
			...
			const activate = (node) => {
				...
				if (node.kind === "chat") props.onPicked(node.row.id);
					...
					if (node.isOpen) setCollapsed(true);
					else openProject(node.group.id);
				} else if (node.kind === "moreChats") setRaised(true);
				else if (node.kind === "moreProjects") setShowAll(true);
			};
			const move = (delta) => {
			...
			const onListKeyDown = (event) => {
				...
				if (event.key === "ArrowDown") {
					...
					if (node !== undefined && node.kind === "project" && !node.isOpen) {
						event.preventDefault();
						openProject(node.group.id);
					}
				} else if (event.key === "ArrowLeft") {
					if (node === undefined) return;
					if (node.kind === "project" && node.isOpen) {
						event.preventDefault();
						setCollapsed(true);
						return;
					}
					if (node.kind === "chat" || node.kind === "moreChats") {
						event.preventDefault();
						setActiveKey("p:" + node.group.id);
						setCollapsed(true);
					}
			...
			if (loading) {
				...
				content_ = results.map((result) => {
					...
					return React.createElement("div", {
						...
						result.group === null || result.group.title === null ? null : React.createElement("span", { className: "dsh-rail-sessions__count" }, result.group.title)
					);
				});
			} else {
				content_ = body;
				...
				return React.createElement(React.Fragment, null,
					React.createElement("div", { className: "dsh-rail-sessions__main" },
						React.createElement("div", { className: "dsh-rail-sessions__mainHead" },
							...
							React.createElement("button", {
								type: "button",
								className: "dsh-rail-sessions__more",
		...
		function RailGlyph(props) {
			...
			/** Abandon a pending or running close: the pointer came back. */
			const cancelClose = () => {
				clearTimers();
			...
			React.useEffect(() => {
				...
				const onPointerDown = (event) => {
					...
					// read as "the pointer left the flyout".
					if (target instanceof Element && target.closest('[role="presentation"]') !== null) return;
					hide();
				};
				...
				const onDocumentLeave = () => scheduleClose();
		...
		function apply(ctx) {
			...
			ctx.effect(() => locale.addLanguage({ id: "ru", label: "Русский", fallback: "en" }), "session-rail: ru language");
